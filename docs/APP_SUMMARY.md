...
## 📝 7. 최근 주요 업데이트 내역
* [2026-04-02]: Firebase Hosting(Cloud Run) 배포 시 API 500 방지 — `firebase-admin`을 **모듈형 초기화**(`firebase-admin/app` 등)로 통일, `FIREBASE_CONFIG`가 있으면 **무인자 `initializeApp()`**(공식 권장), `next.config.mjs`에 `serverExternalPackages: ['firebase-admin']`로 번들 분리.
